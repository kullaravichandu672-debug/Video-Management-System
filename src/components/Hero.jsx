...
if (!movie){
  ...
  const exists = saved.find((m) => m.id === savedMovie.id);
  ...
  return (
    <div className="text-white relative">
        ...
        alt="log-img" className="w-full rounded-2xl h-[480px] object-center object-cover"/>
        <div className='flex space-x-2 md:space-x-4 absolute bottom-3 left-4 md:bottom-8 md:left-10 font-medium'>
            <button 
  onClick={handleSave}
  className='flex justify-center items-center bg-white hover:bg-gray-200 text-[#e50914] py-3 px-4 rounded-full cursor-pointer text-sm md:text-base'>
